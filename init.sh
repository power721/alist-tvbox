...
init() {
  ...
  wget -T 30 -t 2 https://har01d.org/tvbox.zip -O tvbox.zip || \
  cp /tvbox.zip ./
...
wget -T 10 -t 2 https://har01d.org/version.txt -O version.txt
...
wget -T 30 -t 2 https://har01d.org/update.zip -O update.zip
...
if [ ! -f version.txt ]; then
  ...
  if [ "$remote" = "$local" ]; then
    ...
    wget -T 40 -t 2 https://har01d.org/index.zip -O index.zip
