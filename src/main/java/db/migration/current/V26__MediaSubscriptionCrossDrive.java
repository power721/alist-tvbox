...
/**
 * V26:追剧订阅跨盘转存开关 —— media_subscription 增加 cross_drive(BOOLEAN,默认 FALSE)。
 * 默认仅同盘转存(源分享盘 == 目标账号盘,服务端保存式,快而稳);跨盘需订阅显式开启,
 * 或 AList 跨盘秒传配置(Setting {src}_to_{dst},如 quark_to_123)允许该方向。标识符不加引号(见 V21 教训)。
 */
public class V26__MediaSubscriptionCrossDrive extends BaseJavaMigration {

    @Override
    public void migrate(Context context) throws Exception {
        Connection connection = context.getConnection();
        String actualTable = findTable(connection, "media_subscription");
        if (actualTable == null || findColumn(connection, actualTable, "cross_drive") != null) {
            return;
        }
        execute(connection, "ALTER TABLE " + actualTable + " ADD COLUMN cross_drive BOOLEAN DEFAULT FALSE");
    }
