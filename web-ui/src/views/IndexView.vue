  ...
  <div>
    <el-form :model="form" :label-width="labelWidth">
      ...
      <el-form-item label="索引路径">
        <el-input v-model="form.paths" type="textarea" :rows="6" placeholder="每行一个路径" />
      </el-form-item>
    ...
    <el-dialog v-model="settingVisible" title="模板设置">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button v-if="templateId" type="success" @click="saveTemplate(templateId)"
            >更新</el-button
          >
    ...
    <el-dialog v-model="templatesVisible" title="索引模板" width="80%">
      <el-table :data="templates" border style="width: 100%">
        ...
        <el-table-column fixed="right" label="操作" width="140">
          <template #default="scope">
            <el-button link type="primary" size="small" @click="loadTemplate(scope.row)">
              加载
            </el-button>
            <el-button link type="danger" size="small" @click="deleteTemplate(scope.row)">
              删除
            </el-button>
          </template>
        </el-table-column>
...
const loadTasks = () => {
  api.get("/api/tasks?sort=id,desc&size=10&page=" + (currentPage.value - 1)).then((data) => {
    tasks.value = data;
...
const loadTemplates = () => {
  api.get("/api/index-templates?sort=id,desc").then((data) => {
...
const validateIndexFiles = () => {
  api
    .post(
      "/api/index-files/validate?updateExcludePath=" +
        updateExcludePath.value +
        "&depth=" +
        depth.value,
    )
    .then(() => {
...
const saveTemplate = (id: number | null) => {
  ...
  api.post(url, request).then((data) => {
...
const deleteTemplate = (data: any) => {
  api.delete("/api/index-templates/" + data.id).then(() => {
    loadTemplates();
  });
};
...
const handleCancel = (data: any) => {
  api.post("/api/tasks/" + data.id + "/cancel").then(() => {
    loadTasks();
  });
};

const handleDelete = (data: any) => {
  api.delete("/api/tasks/" + data.id).then(() => {
    loadTasks();
  });
};

const cleanTasks = () => {
  api.delete("/api/tasks?days=" + days.value).then(() => {
...
const handleForm = () => {
  ...
  api.post("/api/index", request).then(() => {
    currentPage.value = 1;
    loadTasks();
  });
};
