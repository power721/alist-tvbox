  ...
  <div class="files">
    ...
    <el-dialog v-model="formVisible" :title="'编辑 ' + form.id" width="60%">
      <el-form label-width="140px">
        ...
        <el-form-item label="搜索">
          <a
            :href="'https://www.themoviedb.org/search/' + form.type + '?query=' + form.name"
            target="_blank"
            >{{ form.name }}</a
          >
        </el-form-item>
    ...
    <el-dialog v-model="addVisible" title="添加电影数据" width="60%">
      <el-form label-width="140px">
        ...
        <el-form-item label="搜索">
          <a href="https://www.themoviedb.org/search?query=" target="_blank"
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" :disabled="!form.path || !form.tmId" @click="saveMeta"
            >保存</el-button
          >
        </span>
      </template>
...
const deleteSub = () => {
  ...
  if (batch.value) {
    ...
    api.delete("/api/tmdb/meta/" + form.value.id).then(() => {
      dialogVisible.value = false;
      scrapeVisible.value = false;
      refresh();
    });
...
const syncMeta = () => {
  api.post("/api/tmdb/meta-sync").then(() => {
...
const saveMeta = () => {
  ...
  api.post("/api/tmdb/meta", form.value).then((data) => {
    if (data) {
...
const updateMeta = () => {
  api.post("/api/tmdb/meta/" + form.value.id, form.value).then((data) => {
    if (data) {
...
const load = (value: number) => {
  page.value = value;
  api
    .get("/api/tmdb/meta?page=" + (page.value - 1) + "&size=" + size.value + "&q=" + keyword.value)
    .then((data) => {
...
const loadIndexFiles = () => {
  indexName.value = "";
  api.get("/api/sites/" + siteId.value + "/index").then((data) => {
    index.value = data;
