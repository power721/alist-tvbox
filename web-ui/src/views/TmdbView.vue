    ...
    <el-dialog v-model="formVisible" :title="'编辑 ' + form.id" width="60%">
      <el-form label-width="140px">
        ...
        <el-form-item label="搜索">
          <a :href="'https://www.themoviedb.org/search/' + form.type + '?query=' + form.name" target="_blank">{{
            form.name
          }}</a>
        </el-form-item>
        <el-form-item>
...
const scrape = () => {
  axios
    .post('/api/tmdb/meta/' + form.value.id + '/scrape?name=' + form.value.name + '&type=' + form.value.type)
    .then(({ data }) => {
      if (data) {
...
const scrapeIndex = () => {
  axios
    .post('/api/tmdb/meta-scrape?siteId=' + siteId.value + '&indexName=' + indexName.value + '&force=' + force.value)
    .then(() => {
