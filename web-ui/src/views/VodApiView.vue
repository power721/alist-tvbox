<template>
  <div class="vod">
    ...
    <div>
      <el-input v-model="id" placeholder="vod_id" @keyup.enter="getDetail" />
      <el-button type="primary" @click="getDetail"> 资源详情 </el-button>
    </div>

    <div>
      <el-input v-model="path" placeholder="目录完整路径" @keyup.enter="load" />
...
const getDetail = function () {
  ...
  api.get(getPath(type.value) + "/" + store.token + "?ids=" + id.value).then((data) => {
    config.value = data;
  });
};
...
const load = function () {
  ...
  api.get(getPath(type.value) + "/" + store.token + "?t=" + path.value).then((data) => {
    config.value = data;
  });
};
...
onMounted(async () => {
  ...
  api.get(getPath(type.value) + "/" + store.token).then((data) => {
    config.value = data;
  });
});
</script>
