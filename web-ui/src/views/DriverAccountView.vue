  ...
  <div class="list">
    ...
    <el-dialog v-model="formVisible" :title="dialogTitle" width="60%">
      <el-form :model="form" label-width="120">
        ...
        <el-form-item v-if="supportCookie(form.type)" label="Cookie" required>
          <el-input v-model="form.cookie" type="textarea" :rows="5" @change="getInfo" />
          ...
          <el-button v-if="form.cookie" class="hint" type="primary" @click="getInfo">
            校验Cookie
          </el-button>
        </el-form-item>
        <el-form-item v-if="form.type == 'PAN139'" label="Token" required>
        ...
        <!--        </el-form-item>-->
        <el-form-item v-if="form.type == 'PAN115'" label="Token">
          <el-input v-model="form.token" />
        </el-form-item>
        <el-form-item v-if="form.type == 'QUARK_TV' || form.type == 'UC_TV'" label="Token" required>
          <el-input v-model="form.token" type="textarea" :rows="3" />
          <el-button type="primary" @click="showQrCode"> 扫码获取 </el-button>
        </el-form-item>
        <el-form-item v-if="form.type == 'BAIDU'" label="认证令牌" required>
        ...
        >
          <el-input
            ...
            :placeholder="form.type == 'THUNDER' ? '+86 12345678900' : ''"
          />
        </el-form-item>
        <el-form-item
          v-if="form.type == 'THUNDER' || form.type == 'CLOUD189' || form.type == 'PAN123'"
        ...
        <el-form-item v-if="form.type == 'THUNDER' || form.type == 'CLOUD189'" label="验证码">
          <el-input v-model="form.token" />
        </el-form-item>
        <el-form-item v-if="form.type == 'THUNDER'" label="保险箱密码">
          <el-input v-model="form.safePassword" type="password" show-password />
        </el-form-item>
        <el-form-item label="文件夹ID">
          <el-input v-model="form.folder" />
        </el-form-item>
        <el-form-item v-if="form.type == 'PAN139'" label="类型">
        ...
        <el-form-item v-if="form.type == 'QUARK_TV' || form.type == 'UC_TV'" label="视频类型">
        ...
        <el-form-item v-if="form.type == 'PAN139'" label="家庭云ID">
          <el-input v-model="form.addition.cloud_id" />
        </el-form-item>
        <el-form-item v-if="form.type == 'PAN115'" label="删除码">
          <el-input v-model="form.addition.delete_code" type="password" show-password />
        </el-form-item>
        <el-form-item v-if="form.type == 'PAN115'" label="分页大小">
          <el-input-number v-model="form.addition.page_size" :min="100" :max="1500" />
        </el-form-item>
        <el-form-item v-if="form.type == 'PAN115'" label="请求限速">
          <el-input-number v-model="form.addition.limit_rate" :min="1" :max="4" />
        </el-form-item>
        ...
        <el-form-item v-if="supportProxy(form.type)" label="分片大小">
          <el-input-number v-model="form.addition.chunk_size" :min="64" :max="4096" />
        </el-form-item>
        <el-form-item
          v-if="
            !driverRoundRobin &&
        ...
        <el-form-item v-if="form.type == 'CLOUD189'" label="自动签到">
          <el-switch
        ...
        <span v-if="form.name" style="margin-left: 72px">完整路径： {{ fullPath(form) }}</span>
      </el-form>
      <template #footer>
...
const deleteAccount = () => {
  ...
  api.delete("/api/pan/accounts/" + form.value.id).then(() => {
    load();
  });
};
...
const showQrCode = () => {
  ...
  api.post("/api/pan/accounts/-/qr?type=" + form.value.type).then((data) => {
    qr.value = data;
    qrModel.value = true;
  });
};
...
const showQrCodeForCookie = () => {
  ...
  qrType.value = type;
  api.post("/api/pan/accounts/-/qr?type=" + type).then((data) => {
    qr.value = data;
    qrModel.value = true;
  });
};
...
const getInfo = () => {
  ...
  api.post("/api/pan/accounts/-/info?type=cookie", data).then((data) => {
    if (data && data.name) {
...
const show115QrCode = () => {
  api.get("/api/pan115/token").then((data) => {
...
const loadResult = async () => {
  return api.get(`/api/pan115/result?app=${app.value}&uid=${uid.value}`).then((data) => {
    if (data.error) {
...
const load = () => {
  api.get("/api/pan/accounts").then((data) => {
    accounts.value = data;
  });
};
...
onMounted(() => {
  load();
  api.get("/api/settings/driver_round_robin").then((data) => {
