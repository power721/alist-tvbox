...
- **在线日志**: 实时查看应用日志
- **数据库备份**: 每天自动备份（SQL 06:00、JSON 06:30），JSON 为主、可移植（H2/MySQL/PostgreSQL），SQL 为 H2 兜底；支持手动导出/恢复
...
- **JSON 备份（推荐，可移植）**：每日 06:30，文件名 `database-json-yyyy-MM-dd-HHmmss.zip`。基于 JPA 逐表导出，与数据库无关（H2 / MySQL / 未来 PostgreSQL 通用）。
