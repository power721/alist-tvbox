...
- **海报墙模式**: 瀑布流展示，支持分类筛选
- **网络直播**: 虎牙、斗鱼、B站、网易CC、快手、抖音、Twitch、SOOP实时直播流
- **电报搜索**: 频道搜索、网页搜索、豆瓣数据库浏览
- **追剧订阅**: 订阅剧集自动追更——定时检查更新、链接失效自动换源、缺集自动补齐、可选自动转存到自己的网盘，TvBox端像逛媒体库一样追剧（详见[追剧订阅使用指南](../docs/media-subscription-guide.md)）
...
- **影视首页**: 上传网页即成首页站点（自定义网页源），搜索/继续观看/网盘直开/盘搜/盘检开箱即用
...
内置订阅源：
- **csp_PianDan** - 片单导航（豆瓣/TMDB榜单选剧）
- **csp_XiaoYa** - 小雅搜索源
- **csp_AList** - AList站点浏览
- **csp_Media** - 我的追剧（追剧订阅媒体库）
...
- **csp_TgWeb** - 电报网页搜索
- **csp_FishPanSou** - 鱼佬盘搜
- **csp_FishPanSouGroup** - 盘搜（分组模式）
- **csp_WebHome** - 影视首页（网页首页，支持自定义网页源）
- **csp_Push** - 推送
...
#### 影视首页与自定义网页源
把一个网页做成 TvBox 的首页站点：进站即全屏打开页面，遥控器操作，可直接搜索、继续观看、跳转播放。同一份页面在两类客户端零改动通用——webhtv/fish 按原生首页渲染，原版 FongMi/OK影视 等普通端由内置 `spring.jar` 的 `csp_WebHome` 以全屏 WebView 加载并注入标准 `fm` SDK。

- **开启**：订阅源管理勾选内置源「影视首页」；上传自己的页面即成为自定义网页源
- **上传**：管理页面「文件 → 静态文件」进入 `webhome/pages/` 目录上传 HTML（单文件即可，zip 可带 css/js/图片等相对资源），刷新订阅配置后自动出现在站点列表，可排序/改名
- **页面能力**（页面通过 `fm` SDK 调用，普通端自动补齐）：网络请求（无 CORS 限制）、最近观看、跳转原生详情/搜索/直播/收藏/设置、网盘分享直开（自动带提取码）、盘搜、分享链接有效性检测、防盗链图片代理
- **自动后端**：页面访问公开盘搜站的请求会自动改走本服务（需在配置里设置盘搜地址并启用 token，上游与登录由服务端承担）；盘检同样走服务端批量验证
- 开发与移植规范（fm SDK 契约、兼容红线、常见坑）见[网页开发指南](../docs/webhome-page-dev-guide.md)


### 追剧
订阅一部剧，系统自动追更：定时检查资源更新、链接失效自动换源（播放进度不断）、缺集自动从多个分享补齐，还可自动转存到自己的网盘，告别"收藏夹里的链接全失效"。

- 管理页面「追剧」新建订阅：粘贴豆瓣/TMDB/Bangumi/腾讯/优酷/爱奇艺/B站条目链接自动解析绑定，或按剧名搜索绑定元数据（官方集数、播出日程、完结状态、封面）
- 搜索页、播放记录的「追更」按钮一键订阅；「片单追更」从豆瓣/TMDB热门榜单直接选剧
- 资源搜索覆盖TG频道与站点搜索源：玩偶聚合默认开启，盘链/观影/蜗牛可配自己的账号
- 两种资源模式：挂载追更（免网盘账号）与自动转存（多网盘增量复制）；支持TG通知、事件流、导出/导入
- TvBox端自动多出「我的追剧」内置源（csp_Media）：像逛媒体库一样浏览所有订阅，每集是逻辑链接实时选源，单个源失效自动切换下一个源

详细功能与常见问题见[追剧订阅使用指南](../docs/media-subscription-guide.md)。
...
管理界面登录后进入"直播"页面，可以按平台浏览分类、翻页查看房间并播放直播流。支持关注主播（可粘贴官方直播间地址直接添加）、直播弹幕、平台Cookie配置（抖音风控自愈、SOOP登录观看受限房间、B站登录提高接口配额）。
