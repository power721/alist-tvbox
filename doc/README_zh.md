...
自定义PG包，下载最新的PG包放在/opt/alist-tvbox/pg.zip，点击同步文件按钮。
...
自定义115分享资源：
在/opt/alist-tvbox/pg/lib目录新建文件115share.txt。

复制原文件内容，添加新的分享，点击同步文件按钮。

其它分享类似，在压缩包/opt/alist-tvbox/pg.zip查看分享文件。
...
自定义真心包，下载最新的真心包放在/opt/alist-tvbox/zx.zip，点击同步文件按钮。
...
10. 重置用户名和密码。创建文件/opt/alist-tvbox/atv/cmd.sql，写入下面的内容。重启应用，恢复默认的admin密码。
   ```sql
