...
**⚠️ Critical design constraints (read before implementing):**

1. **Startup ordering.** `ApplicationRunner`/`CommandLineRunner` run *after* the context is refreshed — i.e. after every `@PostConstruct`. All 23 initializer services use `@PostConstruct`, so the restore-state flag MUST be flipped to `pending` *before* context refresh. This is done with an `ApplicationListener<ApplicationEnvironmentPreparedEvent>` registered via `META-INF/spring/org.springframework.context.ApplicationListener.imports` (or `spring.factories`). The actual repository restore still runs in an early bean after the JPA layer is up, but the *flag* is live before any initializer. See Task 7.
2. **Jackson YAML mapper needs `JavaTimeModule`.** The codebase relies on Spring Boot's auto-configured mapper for `java.time` support and registers `JavaTimeModule` nowhere. Any hand-built `new ObjectMapper(new YAMLFactory())` MUST call `.registerModule(new JavaTimeModule())` and disable `WRITE_DATES_AS_TIMESTAMPS`, otherwise `Instant` fields (`exportedAt`, `TmdbMeta.time`, `History`, …) throw `InvalidDefinitionException`.
3. **Entity↔Map serialization must handle relationships.** Included entity `TmdbMeta` has `@ManyToOne Tmdb`. The generic `convertValue(entity, Map)` export pulls Hibernate proxies / nested objects, and restore would reconstruct nested entities. Flatten FK relations to their id (or `@JsonIgnore` + handler override) for every included entity that has a relation. Audit all included entities for `@ManyToOne/@OneToMany` before coding the generic handler.
4. **Restore must be transactional.** Overwrite deletes all included entities then re-inserts; a mid-restore failure without a transaction = permanent data loss. Wrap restore in `@Transactional` (spec requires per-module/per-phase boundaries + fail-fast).
5. **Native-image scan is non-recursive.** `Main.findAllClassesUsingClassLoader` lists one package level only (hence the explicit `dto.bili`, `dto.emby`, … lines). `dto.backup` must be added as its own explicit scan line in `Main.java`. See Task 11.
...
> **Note:** This first test is a plain unit test (no Spring context). Task 2 converts this class into a `@SpringBootTest` integration test that autowires `DatabaseBackupService`. When that happens, add the `@SpringBootTest` (and `@Transactional` for rollback between tests) annotations to the class and keep this pure-serialization test working with its local mapper.
...
- [ ] **Step 1: Convert the test class to a Spring Boot integration test and assert zip export**

Add `@SpringBootTest` + `@Transactional` to the class (keep the existing pure-serialization test from Task 1 — it uses its own local mapper and needs no context). Then add an autowired field and an export test:
...
> Do NOT write a throwaway hand-constructed `new DatabaseBackupService(null, null, …)` test — the service is a Spring bean with a small constructor (registry + repositories); construct it via the context, not by hand.
...
public class DatabaseBackupService {
    ...
    // serialize without it. Disable timestamps so output matches the spec's ISO-8601 example.
...
> **Relationship handling (required).** The injected `objectMapper` MUST carry `JavaTimeModule` (the same configuration as `DatabaseBackupService.yamlMapper`) so `convertValue(entity, Map)` handles `Instant` fields. For any included entity with a JPA relation — confirmed: `TmdbMeta` has `@ManyToOne Tmdb` — the default `convertValue` will serialize a nested object / Hibernate proxy and restore will try to re-persist it. For each such entity, either annotate the relation with `@JsonIgnore` plus a flat `xxxId` column (preferred — `TmdbMeta` already has `siteId`/`tmId`; verify it carries the Tmdb id flatly), or override `exportItems()`/`toEntity()` in that entity's handler to flatten the FK to its id. Audit every included entity for `@ManyToOne`/`@OneToMany` before relying on the generic handler.
...
Add to `DatabaseBackupService.java` (note `@Transactional` — overwrite deletes everything before re-inserting, so the whole restore must roll back as one unit on any failure):
...
private void rebuildIdGenerator() {
    ...
    tables.forEach((table, entityName) -> {
        ...
        // Use a delete+insert upsert (not vendor-specific `merge into`) for cross-database support.
...
> Verify the actual `id_generator` column names against the `@TableGenerator` config in the entities (the standard JPA TABLE generator uses configurable `pkColumnName`/`valueColumnName`); adjust `entity_name`/`next_id` to match before running.
...
> The skeleton above counts every MERGE row as `updated`. Have `mergeEntity` (or the handler) report whether the row already existed so the summary distinguishes `created` vs `updated` — the UI displays this summary. A simple approach: return a small result/boolean from the merge lookup and increment the matching counter.
...
**Why this shape:** the restore-pending flag must be readable by initializer `@PostConstruct` methods, which run *during* context refresh — long before any `ApplicationRunner`. The robust, simple way to guarantee the flag is set in time is to detect the package file in `RestoreState`'s **constructor**. Because every guarded service (Task 8) injects `RestoreState`, Spring constructs `RestoreState` (running its constructor) before those services' `@PostConstruct` execute. No `ApplicationRunner`-set flag — that would be too late.
...
public class RestoreState {
    ...
    // Detect the restore package at construction time so the flag is live before any
    // guarded service @PostConstruct runs (those services inject RestoreState, which forces
    // this bean to be constructed first).
...
> If construction-order detection ever proves fragile (e.g. a guarded service forgets to inject `RestoreState`), the fallback is an `ApplicationListener<ApplicationEnvironmentPreparedEvent>` registered via `META-INF/spring/org.springframework.context.ApplicationListener.imports` that flips a static flag before refresh. Prefer the constructor approach; only escalate if needed.

- [ ] **Step 4: Implement startup runner**

Create `StartupYamlRestoreRunner.java`. It performs the actual restore after the JPA layer is up; the *pending* flag was already set by `RestoreState`'s constructor, so initializer writes have already been skipped by the time this runs.
...
public class StartupYamlRestoreRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        // On failure: do NOT mark completed and do NOT delete the package — keep it for diagnosis
        // and retry on next boot (spec: fail fast, never half-restore silently). The exception
        // propagates and fails startup intentionally.
...
```java
@Test
void shouldSkipInitializerWriteWhenRestoreIsPending() throws Exception {
    Path tempDir = Files.createTempDirectory("yaml-guard");
    Path restoreFile = tempDir.resolve("database-yaml.zip");
    Files.write(restoreFile, new byte[]{1});
    // Constructor detects the package and sets pending=true.
...
- [ ] **Step 2: Add the backup DTO package to the Main.java scan**

`Main.findAllClassesUsingClassLoader` is **non-recursive** — it lists a single package directory only. That is why `dto.bili`, `dto.emby`, `dto.tg`, `dto.pansou`, `dto.sync` each have their own explicit scan line. Scanning `cn.har01d.alist_tvbox.dto` will **not** pick up `cn.har01d.alist_tvbox.dto.backup`.

Add an explicit line in `Main.main` next to the other `dto.*` scans:

```java
classes.addAll(findAllClassesUsingClassLoader("cn.har01d.alist_tvbox.dto.backup"));
```

(The `entity` package is already scanned at line 41, so the included entities themselves are covered.)
