...
> **🆕 详情页背景图高清 + 多图轮播(2026-08-24,参考 atv-player)**:web 端「媒体详情」头部横幅此前只有单张 TMDB `w780`(糊)且静态。①**高清**:TMDB 详情请求加 `append_to_response=images` 零额外请求带回 `images.backdrops`,主图与候选全部用 `original` 尺寸(`media.themoviedb.org/t/p/original`);新增 `MetadataDetails.backdrops`(随 media_metadata payload 持久化,无需迁移):官方主图恒置顶,其余按 atv-player 同款打分(`vote_average×1000 + min(vote_count,1000)×2 + min(width,3840)/20 − |ratio−16/9|×140`)排序去重取 8 张;豆瓣桥接沿用(豆瓣无多图,backdrops 来自 TMDB 回填)。②**存量免刷新升级**:`detail` 组装时对快照里的 `/t/p/w780/` 做纯 URL 改写升 `original`(零网络),`backdrops` 输出 = 候选 ∪ 主图去重(存量单图订阅也得单元素列表,前端零分支);新候选要等下次刷新元数据/在播 6h TTL 重拉落库。③**前端轮播**(`MediaSubscriptionsView.vue`):候选层叠绝对定位 + opacity 0.9s 淡入淡出,4.5s 定时取模切换(atv-player 同节奏),>1 张才启动定时器,后续帧 `new Image()` 预加载防切图闪白,drawer 关闭/组件卸载清理。单测:`TmdbMetadataProviderBackdropTest` 六条(端到端 original/主图置顶投票降序/16:9 惩罚垫底/上限 8/无 images 回落主图/空载荷)+ `MediaSubscriptionFastDetailTest` 三条(存量 w780 升级+候选合并去重代理/单图兜底数组/URL 改写只动 TMDB 段),全量 **880 绿**,web-ui build(vue-tsc)过。
