...
> **🆕 手动添加资源支持已挂载网盘目录·路径资源(#1071,2026-09-08;父母场景「网盘里已有的剧直接选进来」)**:①**新「路径资源」形态**(沿磁力产物先例):`link=path:/xxx` 合成链接、`state=MOUNTED`、`mount_path=目录本身`、`share_id=null` —— 目录本就存在于 AList 无需挂卸,集源行照落,逻辑线路 `msubep-` 实时选源与盘线路 `1@{pid}` 照常按行供流,入账即可播。②**入口 addResource 分流**:分享链接识别失败后尝试路径形态(本站 URL 取路径 %XX 解码、剥 `/dav`、裸路径归一),拒绝根路径/空目录/本订阅主源挂载目录(防 activate 换源把同路径行降级成永不再探的僵尸候选),同路径幂等(重复提交=刷新目录)、RETIRED/REMOVED 复活;盘类型=x_storages 挂载点逐级上溯(`AListLocalService.findStorageDriverByPath`)+`DriveId.fromDriverName` 驱动名映射(115 Cloud→115 等,未知驱动只并逻辑线路)。③**即时入账 registerPathResource**:列举→sanitize(衔接链口径)→异剧/时长门禁(磁力产物同款)→落集源行+GAP_FILLED,失败原因 400 透传前端弹层。④**网页「添加资源」双 tab**:「网盘目录」选择器为默认主入口(`GET /api/media-subscriptions/drive-dirs` 一层懒加载,仅目录名+排序+拒 `..`,el-tree 根层=已挂载存储,「添加此目录」走同一 POST);「分享链接」表单保留;路径行加「路径」角标、隐藏「转主源」。⑤**巡检适配**:refreshAuxMounts 原位重列承接目录增长(父母网盘续存新集自动跟集)、归属复核豁免(手动事实=钉选同款)、换血排除、`usableAsPrimary` 恒 false+转主源入口同步拒绝(无分享链接挂不到固定路径,只作补缺线路)、恢复=原位重新入账(不回候选池,探测流救不回)。单测 23 新增,存量 FollowTest URL fixture 一处调整,全量 1612 绿(commit 7d6914ce)。

> **🆕 豆瓣片单条目改独立 subject id 类型 db:{id}(2026-09-06;用户指出:rexxar 榜单接口本就返回 `id`,且 s: 形态把标题嵌进 vod_id,标题含 `&` 会被爬虫端裸拼的 GET 参数截断)**:①**新 vod_id 类型 `db:{纯数字}`**(`PianDanService.doubanSubjectId`)——id 独立成类型不嵌任何标题字符,`&`/`#`/空格 从根上免疫;`s:{标题}[@{年}]` 保持原样仅作无 id 条目兜底。三个生产点全改发 db::`getMovieDetail`(rexxar 榜单 recent_hot/subject_collection/suggestion 三端点 items 均带 id,线上实测)、`getLocalMovieList`/`getRandomMovie`(本地 `Movie.id` 即豆瓣 subject id)。②**db: 详情两级解析**:本地库 `localDoubanDetailById`(findById 直取,id 同命名空间命中即该条目,零网络)→ 未收录的榜单新片回落 `PianDanService.doubanSubjectDetail`(rexxar 条目接口 tv 404 再试 movie 两跳,取 title/year/genres/countries/languages/directors/actors/rating/pic.large/intro,5min 短缓存共享实例拷贝防污染)→ 双 miss 400(与 tmdb 分支同口径);两处入口(TvBox `pianDanDetail`/网页 `navigationDetail`)同链路,`isPianDanId` 认 db:。③**订阅绑 id 直绑**:`PianDanEntry` 加 `doubanId` 分量(db: vodId 直接解析),`bindDoubanMeta` 首路直绑 `doubanId/metaProvider=douban/metaId` 零网络零消歧;载荷空名时按 id 解析标题(本地直取→rexxar 兜底,镜像 tmdbMeta);无 id 的 s: 条目回落本地名匹配→suggest 两级不变;`listDouban` 的 s: 搜索分支同步剥 `@{年}` 后缀只认裸标题。单测 3 条新增(db: 纯数字形态/localDoubanDetailById 直取三态/控制器 db: 本地优先→在线兜底→双 miss 400→非法格式 400),全量 1546 绿。

> **🆕 检查更新落完结 + 集齐总数完结路(2026-09-06;起因:师兄太稳健 sub44 全 30 集收齐,官方已播滞后 26/30+RETURNING+无下集排播,四条完结路全灭卡 ACTIVE;用户定规:点「检查更新」就应修复,不要被迫跑巡检)**:①`shouldAutoEnd` 增第四路 endedByCollectedAll——集齐 `effectiveTotalEpisodes`(>0)且 `nextAirTime==null` 即完结:官方已播统计滞后于资源现实(全季网盘已有、元数据已播停在 26/30)时季播完路 aired≥total 永不满足,但本季已无可追之物;下集排播非空(官方登记未播完)或未集齐不触发,官方扩总数/已播上调由 reopenEnded 正常重开。②`checkUpdateInternal` 快照刷新落库后评估 `shouldAutoEnd`(live 集源行计数),达标即转 ENDED+事件「已完结(共 N 集)(检查更新)」并以完结文案作 msg 回执——轻查秒级完成,巡检(列目录/搜索/补缺)不再是对齐状态的唯一通道;PAUSED(用户主动冻结)不迁移。③同族第二案龙之家族 S3 sub58(officialEpisodes 恒 null+status UNKNOWN:豆瓣 rexxar 无已播数、TMDB 名称桥未命中,封面留豆瓣图床即桥未接的铁证)同被第四路覆盖(8/8 收齐+无下集排播);详情页 status 徽标补订阅 ENDED 覆盖(此前只认 isSeasonAiredOut,该形态 officialEpisodes=null 恒 false,完结后徽标仍挂 UNKNOWN)。单测 2 条(第四路三态门禁/检查更新完结触发含事件落库),全量 1541 绿。

> **🆕 官方已播对总集数夹紧统一口径(2026-09-06;起因:瑞克 S9 订阅 18 完结后被污染已播反复重开,ACTIVE 空转 12 天)**:B站 `refineAiredCount` 季盲匹配(搜索基名「瑞克和莫蒂」命中 S1 条目)把 S1 口径已播 11 取大灌进总 10 的 S9 详情,快照落库不夹 → 已完结订阅(08-23 季播完路正常完结)被 `shouldReopen`(官方 11 > 本地 10)误判「官方集数上调(10 → 11)」重开(08-25 线上事件),再完结被季播完路 `collected≥officialEpisodes(11)` 卡死(当时 expected/manual 均空),「检查更新」直读 details 也把污染值报成「本地缺第 11 集」假缺口。修复三处:①`applyMetadataSnapshot` 总数先落、已播对 `officialTotal` 夹紧(与 computeMissing base/详情页已播两处既有夹紧同规;柯南 B站领先取大特性不受影响——领先值本就被总数夹住);②`checkUpdateInternal` 已播改读夹紧后的实体快照,不再直读 details;③`mergeTmdbDetails` 桥接出口对豆瓣总数夹住(豆瓣总数=条目身份权威,上游修正总数后自然放开)。根因(`refineAiredCount` 不辨季、基名匹配跨季冒领)维持既有「不根治」决策,损害已被三处夹紧全面中和;存量订阅部署后首轮 refreshMetadata 自愈(11→10 后完结判定/重开判定/检查更新全链恢复)。单测 2 条(快照夹紧三分支/桥接出口三形态),全量 1539 绿。
...
>
> **玩偶聚合搜索源(2026-08-22;2026-09-20 与 py 9880ef2 同步:移除六死站 欧歌/至臻/二小/蜡笔/虎斑/小斑,新增表哥/花卷)**:atv-spiders `py/玩偶聚合.py` 移植为 `WanouSearchService`,成为追剧搜索源之一(§4.6 第四源)。7 个玩偶系 MacCMS 网盘站并行搜索(站点优先级玩偶>多多>木偶>快映>闪电>表哥>花卷),卡片标题归一化粗匹配(剥 4K/站名噪声与集数/季/年份标记,双向包含,精确过滤仍由 `matchesTitle` 把关)后抓详情页(`module-row-info p` 文本;花卷走 `down-card-url` 文本、搜索卡片为海报形状 `module-card-item-poster`),提取分享链接(修 `hhttps://` 复制瑕疵、URL 停在中文/全角字符、行内提取码折 `?password=` 供 `ShareService.parsePassword` 消费),`Message.parseType` 复用盘型识别产出与 TG 同构的候选。**域名治理**:静态表仅种子兜底,`pan-site-monitor`(默认 `https://pan-site-monitor.douer.me/api/data`,`app.subscription.wanou-monitor-url`)每 6h 下发按延迟排序的可达地址(监控键为中文名,欧歌↔欧哥),拉取失败 10min 重试;请求逐域名 failover+成功粘滞,CF 挑战页(200+challenges.cloudflare.com)识别为失败保证轮转,全域名失败站点冷却 30min。接入点:`fillPool`/`preview` 经 `searchAllSources` 与 TG 三级回退结果按 link 去重合并(玩偶源失败仅告警不影响 TG)。配置:`wanou-enabled`(默认开)/`wanou-max-detail-pages`(单站详情页上限,默认 3)/`wanou-timeout-seconds`(总超时 45s)。单测:`WanouSearchServiceTest`(解析/盘型/failover 粘滞/监控刷新/跨站去重/CF 判定);实测冒烟:7+ 站可达,「难哄」召回 34 条(夸克/UC/阿里/115/天翼),20s 内完成。
...
> **观影搜索源(2026-08-22)**:atv-spiders `py/观影.py` 移植为 `GuanYingSearchService`(§4.6 第六源)。需登录的分享聚合站,多镜像互为镜像(教父.com/星际穿越.com/hgeme.com 等,清单以官方地址发布页「挂了.com」check.js 的 urlData 为准——2026-09-20 楚门的世界/泰坦尼克号/阿甘正传被退役、新增 ASCII 域 hgeme.com;www 形态才是服务域名、裸域全路径 302 到 www,www A 记录 TTL≈1s 轮换反封锁任一时刻常有镜像解析到 0.0.0.0,解析层剔死记录;已知镜像全失败时自动抓发布页自发现最新清单再试一轮,10min 节流,自定义 `guanying_host`(可能私有部署)不混入防 Cookie 跨实例泄漏)。**凭证必须用户自配**(与盘链同策略):Setting `guanying_username`+`guanying_password`(FormBody 登录 `/user/login`:code/siteid=1/dosubmit=1/cookietime/username/password;captcha 响应提示改配 Cookie)或 `guanying_cookie`(种子进共享 Cookie 状态);追剧设置对话框「观影搜索源」分组;未配置 → 源静默关闭。**反爬两道**:①PoW——响应呈挑战特征(JSON code=419 / refresh=1+验证 / 浏览器验证已过期 / pow.worker / filejin;**`_obj.` 在场优先判为正常数据页**,否则 nologin 壳页里的 filejin 静态域名会误判)时 `/res/pow` 取 {N,x,t} 算 `y=x^(2^t) mod N`(`BigInteger.modPow`,与 Python pow 对照测试)提交换 browser_verified;②登录态——nologin/未登录 响应自动重登一次,失败 5min 冷却。请求管线:多镜像逐个 failover(404/5xx/4xx 换下家,成功镜像粘滞),Set-Cookie 全镜像共享合并(Max-Age<=0/deleted = 删除)。搜索:HTML 内嵌 `_obj.search={l:{i,title,d,year,info}}` 平行数组,**空则回退 `/res/search_suggest`(实测匿名可用,未登录也拿得到条目)**;盘链 `/res/downurl/{type}/{rid}` 的 `panlist.url/name/p` 平行数组,提取码折 `?password=`(该站约定统一 password,与盘链按盘折 pwd= 不同);downlist 磁力与未知盘丢弃。单测 11 个:PoW 四组对照值/挑战判定(含 _obj 优先序)/解析双路径/PoW 恢复重试/登录/假 Cookie 失效路径。**坑:`Map.of` 迭代顺序 JVM 间随机(ImmutableCollections SALT),拼查询串必须 LinkedHashMap**,否则桩测试偶发 miss(全量首跑即翻车,改保序+按路径匹配后三连绿)。实测冒烟:8 镜像中 2 个 DNS 已死其余互通,suggest 匿名返回「难哄」tv/g22b,downurl 假 Cookie 403 全镜像轮转后正确返回空。
...
| 玩偶聚合站 | `WanouSearchService`(7 个玩偶系 MacCMS 网盘站:玩偶/多多/木偶/快映/闪电/表哥/花卷;表哥/花卷 2026-09-20 接入,监控服务暂无此二站键走纯种子域名) | 网页抓取:并行按站搜索 → 卡片标题粗匹配 → 抓详情页提取网盘分享链接(行内提取码折 `?password=`);域名由监控服务下发最新可达地址,逐域名 failover + 成功粘滞;CF 挑战页识别为失败 | 候选池补充源:`fillPool`/`preview` 与 TG 三级回退结果按 link 去重合并(`searchAllSources`),实测 7+ 站可达、单剧 30+ 分享 |
